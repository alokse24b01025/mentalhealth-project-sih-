...
// This is the themed card, now with download buttons.
const ResourceCard = ({ resource }) => {
  ...
  const renderContent = () => {
    switch (type) {
      case 'video':
        return <iframe className="w-full h-48 rounded-lg" src={url} title={title} frameBorder="0" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen></iframe>;
      case 'guide':
        return (
  ...
  return (
    <div className="border border-green-100 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transform transition-all duration-300 bg-white p-6 flex flex-col">
      ...
      {/* --- NEW: Download Links Section --- */}
      {downloads && (
        <div className="border-t pt-4">
          <h4 className="text-sm font-semibold text-gray-600 mb-2">Download Options:</h4>
...
// This is the main page component with the corrected layout and search feature.
const ResourcesPage = () => {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredResources = useMemo(() => {
    if (!searchTerm) return mockResources;
      ...
      <main className="flex-1 overflow-y-auto p-8 pt-40">
        ...
        {filteredResources.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="mt-2 text-gray-500">We couldn't find any resources matching your search.</p>
          </div>
        )}
      </main>
      <Footer />
