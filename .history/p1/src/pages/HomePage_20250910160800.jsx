...
// --- NEW: Custom Hook to detect when an element is on screen ---
...
// ... (coreFeatures, FeatureCard, studentProfiles, ProfileCard, vlogData, VlogCard, newsData, NewsCard, OrganizationContactForm components remain the same) ...
const coreFeatures = [
...
const HomePage = () => {
  // --- NEW: Create a ref and visibility state for each section ---
  const [featuresRef, featuresIsVisible] = useIntersectionObserver({ threshold: 0.1 });
  const [profilesRef, profilesIsVisible] = useIntersectionObserver({ threshold: 0.1 });
  const [vlogsRef, vlogsIsVisible] = useIntersectionObserver({ threshold: 0.1 });
  const [contactRef, contactIsVisible] = useIntersectionObserver({ threshold: 0.1 });
    ...
    <>
      <style>
        {`
          ...
          /* Other keyframes for cards can be removed as we now use transitions */
        `}
      </style>
      ...
      <div className="min-h-screen flex flex-col bg-green-50">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <section className="text-center h-[450px] flex items-center justify-center bg-gradient-to-br from-green-100 to-green-200 overflow-hidden">
            <div className="px-4">
              <h2 
                className="text-4xl md:text-5xl font-extrabold text-green-900 transition-all duration-1000"
              >
                Your Journey to Mental Wellness Starts Here
              </h2>
              <p 
                className="mt-4 text-lg text-green-800 max-w-2xl mx-auto transition-all duration-1000 delay-200"
              >
          ...
          <section 
            ref={featuresRef}
            className={`py-16 bg-white transition-all duration-1000 ease-out ${featuresIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          <section 
            ref={profilesRef}
            className={`py-16 px-4 bg-green-50 transition-all duration-1000 ease-out ${profilesIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          <section 
            ref={vlogsRef}
            className={`py-16 px-4 bg-white transition-all duration-1000 ease-out ${vlogsIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          <section 
            ref={contactRef}
            className={`py-16 px-8 bg-green-50 transition-all duration-1000 ease-out ${contactIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
