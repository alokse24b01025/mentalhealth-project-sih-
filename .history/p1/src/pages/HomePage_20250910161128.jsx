...
// --- UPDATED: Custom Hook to detect when an element is on screen within a specific scrollable container ---
const useIntersectionObserver = (options) => {
  const [ref, setRef] = useState(null);
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    // Ensure we have both the element to observe and the scrollable root container
    if (!ref || !options.root.current) {
      return;
    }
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      root: options.root.current, // IMPORTANT: Use the scrolling <main> element as the root
...
// ... (coreFeatures, FeatureCard, studentProfiles, ProfileCard, etc. components remain the same) ...
const coreFeatures = [
...
const ProfileCard = ({ name, location, quote, imageUrl }) => (
    <div className="bg-white p-6 rounded-xl shadow-lg flex flex-col items-center text-center transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
...
const VlogCard = ({ title, author, imageUrl }) => (
...
const NewsCard = ({ headline, source }) => (
  <div className="bg-green-100 p-6 rounded-lg border-l-4 border-green-500 flex items-start gap-x-4">
...
const HomePage = () => {
  const mainScrollRef = useRef(null); // Ref for the scrolling <main> element

  // Pass the mainScrollRef as the root for the observer
  const [featuresRef, featuresIsVisible] = useIntersectionObserver({ root: mainScrollRef, threshold: 0.2 });
  const [profilesRef, profilesIsVisible] = useIntersectionObserver({ root: mainScrollRef, threshold: 0.1 });
  const [vlogsRef, vlogsIsVisible] = useIntersectionObserver({ root: mainScrollRef, threshold: 0.1 });
  const [contactRef, contactIsVisible] = useIntersectionObserver({ root: mainScrollRef, threshold: 0.1 });
  ...
  return (
    <>
      ...
      <div className="h-screen flex flex-col bg-green-50">
        <Header />
        {/* --- UPDATED: Attach ref to the scrollable container --- */}
        <main ref={mainScrollRef} className="flex-1 overflow-y-auto">
          {/* Hero Section */}
          <section className="text-center h-[450px] flex items-center justify-center bg-gradient-to-br from-green-100 to-green-200 overflow-hidden">
            <div className="px-4">
              <h2 
                className="text-4xl md:text-5xl font-extrabold text-green-900"
              >
                Your Journey to Mental Wellness Starts Here
              </h2>
              <p 
                className="mt-4 text-lg text-green-800 max-w-2xl mx-auto"
              >
                Confidential, accessible, and tailored psychological support for college students.
              </p>
            </div>
          </section>

          {/* Scrolling Features Section with Scroll Animation */}
          <section 
            ref={featuresRef}
            className={`py-16 bg-white transition-all duration-700 ease-out ${featuresIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          {/* Student Profiles Section with Scroll Animation */}
          <section 
            ref={profilesRef}
            className={`py-16 px-4 bg-green-50 transition-all duration-700 ease-out ${profilesIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          {/* Vlogs & News Section with Scroll Animation */}
          <section 
            ref={vlogsRef}
            className={`py-16 px-4 bg-white transition-all duration-700 ease-out ${vlogsIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
          ...
          {/* Organization Contact Section with Scroll Animation */}
          <section 
            ref={contactRef}
            className={`py-16 px-8 bg-green-50 transition-all duration-700 ease-out ${contactIsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          >
